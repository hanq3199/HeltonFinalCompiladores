...
( ANY ) {lexeme=yytext(); return Any;}
(ARRAYLEN) {lexeme=yytext(); return Arraylen;}
(ARRAY) {lexeme=yytext(); return Array;}
(CLUSTER) {lexeme=yytext(); return Cluster;}
( COLUMN ) {lexeme=yytext(); return Column;}
(CONSTAINS) {lexeme=yytext(); return Contains;}
(CONVERT) {lexeme=yytext(); return Convert;}
(DAY) {lexeme=yytext(); return Day;}
...
(NCHAR) {lexeme=yytext(); return Nchar;}
...
( NVARCHAR ) {lexeme=yytext(); return Varchar2;}
